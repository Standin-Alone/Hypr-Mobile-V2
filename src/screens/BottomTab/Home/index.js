...
export default class Home extends React.Component {
  ...
  viewPost = item => {
    this.props.navigation.navigate(constants.ScreenNames.Social.VIEW_POST, {
      post: item,
  ...
  viewProfile = item => {
    this.props.navigation.navigate(
      constants.ScreenNames.Social.VIEW_PROFILE,
      item,
    );
  };
      ...
      <>
        {index == 0 && (
          <View style={{flexDirection: 'row'}}>
            <TouchableOpacity
              onPress={this.handleOpenAddStories}
              style={{padding: 15}}>
            ...
            <View style={{top: constants.Dimensions.vh(2)}}>
              <FlatList
                ...
                contentContainerStyle={{
                  paddingRight: constants.Dimensions.vw(10),
                }}
          ...
          {item.filenames.length > 0 ? (
            ...
            item.product_link ? (
              <Components.SocialPostProductCard
                ...
                isHype={item.hypes.some(
                  hypeItem => hypeItem.user_id == this.state.userId,
                )}
  ...
  handleViewStory = (item, stories) => {
    this.props.navigation.navigate(constants.ScreenNames.Social.STORIES, {
      story: item,
      stories: stories,
    });
  };
  ...
  handleOpenAddStories = () => {
    this.props.navigation.navigate(constants.ScreenNames.Social.CAMERA, {
      addType: 'stories',
    });
  };
    ...
    return (
      <>
        <Components.SocialHeader
          onCreatePost={() =>
            this.props.navigation.navigate(
              constants.ScreenNames.Social.CAMERA,
              {addType: 'post'},
            )
          }
          goToMessenger={() =>
            this.props.navigation.navigate(
              constants.ScreenNames.Social.MESSENGER,
            )
          }
              ...
              <FlatList
                ...
                onEndReached={({distanceFromEnd}) => {
                  if (distanceFromEnd > 0) {
                    if (
                      this.state.posts.length == this.state.currentPage ||
