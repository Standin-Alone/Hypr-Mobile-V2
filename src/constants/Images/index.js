import React from 'react';

const loginCover = require('./images/login-cover.jpg');
const quartaLogo1 = require('./images/quarta/quarta-logo-1.png');
const quartaLogo2 = require('./images/quarta/quarta-logo-2.png');
const quartaLogo2Light = require('./images/quarta/quarta-logo-2-light.png');
const quartaLogo3 = require('./images/quarta/quarta-logo-3.png');
const hyprLogo = require('./images/hypr-logo-v2.png');
const hyprLogoNew = require('./images/hypr-logo-final.png');
const hyprLogoOnly = require('./images/hypr-logo-only.png');
const addressIcon = require('./images/address-icon.png');
const userProfile = require('./images/user-profile.jpg');
const myOrder = require('./images/my-order.png');
const noDataBackground = require('./images/no-data-bg.jpg');
const socialPageBackground = require('./images/SocialPageBackground.png');
const hype = require('./images/social-menu-card/hype.png');
const unhype = require('./images/social-menu-card/unhype.png');
const cashBack = require('./images/cash-back.png');
const signIn = require('./images/sign-in.gif');
export default {
  loginCover,
  hyprLogo,
  addressIcon,
  userProfile,
  myOrder,
  noDataBackground,
  socialPageBackground,
  hyprLogoNew,
  hyprLogoOnly,
  hype,
  unhype,
  cashBack,
  signIn,
  quartaLogo1,
  quartaLogo2,
  quartaLogo2Light,
  quartaLogo3,
};
